...
def highest_rating_movies(userid, num):
    conn = sqlite3.connect('filmFinder/database_files/filmfinder.db')
    c = conn.cursor()
    c.execute(
        'SELECT id, title, release_date, genres, poster_path, vote_average, production_countries FROM FILMS WHERE vote_count > 5 ORDER BY vote_average DESC LIMIT ?', (num,))
    rows = c.fetchall()
    c.execute(
        f" SELECT id, casts, crew FROM CREDITS WHERE id in (SELECT id FROM FILMS WHERE vote_count > 5 ORDER BY vote_average DESC LIMIT ?)", (num,))
    creditrow = c.fetchall()
    if userid:
        rows_dict = [{'id': rows[x][0], 'title':rows[x][1], 'release_date':rows[x][2], 'genres':genre_process(
            rows[x][3]), 'poster':rows[x][4], 'vote_average':get_movie_avg_rating(userid, int(rows[x][0])), 'production_countries': re_process(rows[x][6]), 'casts':re_process(creditrow[x][1]), 'crew':creditrow[x][2]} for x in range(num)]
    else:
        rows_dict = [{'id': rows[x][0], 'title':rows[x][1], 'release_date':rows[x][2], 'genres':genre_process(
            rows[x][3]), 'poster':rows[x][4], 'vote_average':rows[x][5], 'production_countries': re_process(rows[x][6]), 'casts':re_process(creditrow[x][1]), 'crew':creditrow[x][2]} for x in range(num)]
    return rows_dict
...
def genenal_search(conditions, mode, offset):
    ...
    order_str = ''
    if mode == 0:  # count
        order_str = f' ORDER BY VOTE_COUNT DESC LIMIT 100 '
    elif mode == 1:
        if filter_list == '':
            order_str = f' WHERE VOTE_COUNT >= 50 ORDER BY VOTE_AVERAGE DESC LIMIT 100 '
        else:
            order_str = f' AND VOTE_COUNT >= 50 ORDER BY VOTE_AVERAGE DESC LIMIT 100 '
...
def general_search_test():
    ...
    test_list.append(
        ['Matrix', ['Action', 'Thriller'], None, None, None, None])
...
def advanced_search_test():
    ...
    test_list.append([None, 'Stephen Spielberg',
                      None, [], None, 1999, 2002, 3])
    ...
    test_list.append(['bridge', 'Steven Spielberg',
                      'TOM HANKS', [], None, None, None, None])
...
# print(highest_rating_movies(10))
